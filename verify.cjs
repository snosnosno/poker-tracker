...
check("handToText 홀덤: 보드 미입력 시 ? ? ? 없이 라벨+액션 한 줄", () => {
  ...
  // Turn: 2인 헤즈업 → 첫액션폴드도 헤즈업 예외로 표시
  if (!/Turn: Q9 BET/.test(txt)) throw new Error("Turn BET 누락:\n" + txt);
  if (!/Turn:.*FOLD/.test(txt)) throw new Error("헤즈업 Turn 첫액션폴드 표시 안됨:\n" + txt);
});
...
check("버그수정: computeActionablePlayers — 아직 액션 없는 스트리트에서 TypeError 없음", () => {
  ...
  const hand = {
    ...
    streetList: ["PREFLOP","FLOP","TURN","RIVER"],
    seats: [
      { id: 0, name: "A", position: "D" },
      { id: 1, name: "B", position: "BB" },
    ],
    cardCount: 2, board: [null,null,null,null,null], roundHole: {},
    streets: {
      PREFLOP: [
        { seatId: 0, playerName: "A", position: "D", action: "call" },
        { seatId: 1, playerName: "B", position: "BB", action: "check" },
      ],
      // FLOP 키 자체가 없음 — 이전에는 undefined.forEach() 크래시
    },
  };
  let result;
  try {
    result = I.computeSortedActionable(hand, 1); // streetIdx=1 = FLOP
  } catch (err) {
    throw new Error("FLOP 미입력 시 크래시: " + err.message);
  }
  if (!Array.isArray(result)) throw new Error("배열 반환 기대");
});

check("버그수정: 헤즈업 플랍 첫액션폴드 표시(헤즈업 예외)", () => {
  installGlobals({});
  const I = loadInternals(src);
  const hand = {
    gameType: "holdem",
    ...
    cardCount: 2, board: ["Ah","Kd","2c",null,null], roundHole: {},
    streets: {
      PREFLOP: [
        { seatId: 0, playerName: "A", position: "D", action: "call" },
        { seatId: 1, playerName: "B", position: "BB", action: "check" },
      ...
      FLOP: [
        { seatId: 1, playerName: "B", position: "BB", action: "bet", amountText: "10K" },
        { seatId: 0, playerName: "A", position: "D", action: "fold" },  // 첫액션폴드지만 헤즈업 → 표시해야
      ],
      TURN: [], RIVER: [],
    },
    winnerName: "B", winnerSeatId: 1,
...
// ── 헤즈업 스터드 순서 버그 회귀 ────────────────────────────────
check("헤즈업 스터드: studFirstSeat 선택 시 computeNextToAct가 해당 플레이어 반환", () => {
  const I = loadInternals(src);
  const hand = {
    gameType: "stud7",
    streetList: ["3RD","4TH","5TH","6TH","7TH"],
    seats: [{ id: 0, name: "Alice" }, { id: 1, name: "Bob" }],
    studCards: {},
    studFirstSeat: { "3RD": 1 }, // Bob(id=1)이 먼저
    streets: { "3RD": [], "4TH": [], "5TH": [], "6TH": [], "7TH": [] },
    holeCards: {},
  };
  const next = I.computeNextToAct(hand, 0);
  if (!next || next.id !== 1) throw new Error(`Bob(id=1) 기대, 실제: ${JSON.stringify(next)}`);
});

check("헤즈업 스터드: studFirstSeat 미선택 시 기본 좌석순(id=0) 반환", () => {
  const I = loadInternals(src);
  const hand = {
    gameType: "stud7",
    streetList: ["3RD","4TH","5TH","6TH","7TH"],
...
check("스터드 3RD: bringin+call+call → bringin 플레이어 옵션(complete 가능)", () => {
  ...
  const hand = {
    gameType: "stud7",
    streetList: ["3RD","4TH","5TH","6TH","7TH"],
    seats: [{ id: 0, name: "Alice" }, { id: 1, name: "Bob" }, { id: 2, name: "Carol" }],
    studCards: {}, holeCards: {},
    studFirstSeat: { "3RD": 0 },
    streets: {
      "3RD": [
        { seatId: 0, action: "bringin" },
  ...
  const next = I.computeNextToAct(hand, 0);
  if (!next || next.id !== 0) throw new Error(`Alice(id=0) 옵션 기대, 실제: ${JSON.stringify(next)}`);
});

check("스터드 3RD: bringin+call+call+check → 라운드 완료(null)", () => {
  const I = loadInternals(src);
  const hand = {
    gameType: "stud7",
    streetList: ["3RD","4TH","5TH","6TH","7TH"],
    ...
    streets: {
      "3RD": [
        ...
        { seatId: 1, action: "call" },
        { seatId: 2, action: "call" },
        { seatId: 0, action: "check" },
      ],
      "4TH": [], "5TH": [], "6TH": [], "7TH": [],
    },
  };
  const next = I.computeNextToAct(hand, 0);
  if (next !== null) throw new Error(`null(라운드완료) 기대, 실제: ${JSON.stringify(next)}`);
});

check("헤즈업 스터드: 브링인 후 다른 플레이어 차례", () => {
  const I = loadInternals(src);
  const hand = {
    gameType: "stud7",
    streetList: ["3RD","4TH","5TH","6TH","7TH"],
    seats: [{ id: 0, name: "Alice" }, { id: 1, name: "Bob" }],
    studCards: {},
    studFirstSeat: { "3RD": 1 }, // Bob(id=1)이 브링인
    streets: {
      "3RD": [{ seatId: 1, action: "bringin", amountText: "2K" }],
      "4TH": [], "5TH": [], "6TH": [], "7TH": [],
    },
    holeCards: {},
  };
